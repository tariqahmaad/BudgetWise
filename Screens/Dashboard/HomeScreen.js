...
const HomeScreen = ({ navigation }) => {
    ...
    const performCleanup = async () => {
      try {
        ...
        if (deletedCategories.length > 0) {
          console.log(
            `[HomeScreen] Cleaned up ${deletedCategories.length} empty categories`
          );
        }
      } catch (error) {
  ...
  // Add focus listener to refresh user data when returning to HomeScreen
  useEffect(() => {
    const user = getUser();
    if (!user) return;
    ...
    const processedData = rawCategories
      .map((cat) => {
        ...
        if (total > 0) {
          ...
          descriptionStr =
            count > 1 ? `${count} expenses` : desc || "1 expense";
        }
    ...
    return (
      <View style={styles.transaction}>
        ...
        <View style={styles.transactionAmountContainer}>
          ...
          >
            {item.amount
              ? formatAmountWithCurrency(parseFloat(item.amount), {
                  showCents: false,
                })
